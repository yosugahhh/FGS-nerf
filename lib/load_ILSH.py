...
# midified from llff data loading
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False):
    ...
    maskdir = os.path.join(basedir, 'mask' + sfx)
    if not os.path.exists(imgdir):
    ...
    mask = [imageio.imread(f) for f in maskfiles]
    mask = np.stack(mask, -1)

    print('Loaded image data', imgs.shape, poses[:,-1,0])

    if not load_depths:
        return poses, bds, imgs, mask
    ...
    return poses, bds, imgs, mask, depths
...
def load_ILSH_data(basedir, factor=1, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, load_depths=False):

    poses, bds, imgs, masks, *depths = _load_data(basedir, factor=factor, load_depths=load_depths) # factor=8 downsamples original imgs by 8x
...
#             zloc = np.percentile(tt, 10, 0)[2]
    ...
    return images, depths, poses, bds, render_poses, i_test, masks
