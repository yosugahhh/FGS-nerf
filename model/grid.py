...
class DenseGrid(nn.Module):
    def __init__(self, channels, world_size, xyz_min, xyz_max, **kwargs):
        ...
        self.xyz_min = xyz_min.to('cuda')
        self.xyz_max = xyz_max.to('cuda')
    ...
    def set_alpha(self, xyz, alpha):
        ...
        '''
        # Normalize xyz to [0, world_size-1]
        ind_norm = ((xyz - self.xyz_min) / (self.xyz_max - self.xyz_min)) * (torch.tensor(self.world_size) - 1)
        ind_norm = ind_norm.round().long()  # Round to nearest integer and convert to long for indexing

        # Clamp indices to be within the valid range
        for i in range(3):
            ind_norm[:, i] = torch.clamp(ind_norm[:, i], 0, self.world_size[i] - 1)

        # Flatten grid for easier indexing
        flat_grid = self.grid.view(self.channels, -1)

        # Compute linear indices
        ind_linear = ind_norm[:, 0] * (self.world_size[1] * self.world_size[2]) + \
                     ind_norm[:, 1] * self.world_size[2] + \
                     ind_norm[:, 2]

        # Set alpha values
        with torch.no_grad():  # Ensure no gradients are tracked
            flat_grid_copy = flat_grid.clone()  # Clone to avoid in-place operation
            flat_grid_copy[:, ind_linear] = alpha
            self.grid.data = flat_grid_copy.view(self.grid.shape)

        # Reshape grid back to original shape
        self.grid.data = flat_grid.view(self.grid.shape)
...
class MaskGrid(nn.Module):
    def __init__(self, path=None, mask_cache_thres=None, mask=None, xyz_min=None, xyz_max=None):
        ...
        if path is not None:
            ...
            alpha = F.max_pool3d(st['model_state_dict']['alpha.grid'], kernel_size=3, padding=1, stride=1)
